...
class PDFTextStripperWithCoordinates(private val regexesToIgnore: List<Regex>) : PDFTextStripper() {
  ...
  override fun writeString(text: String?, textPositions: MutableList<TextPosition>?) {
    ...
    val shouldIgnore = regexesToIgnore.any { it.containsMatchIn(text.toString()) }
    if (shouldIgnore) {
      return
    }
    val clusterableTextPositions = textPositions?.map { ClusterableTextPosition(it) }
    ...
    if (clusters.size > 1) {
      ...
      clusters.map { cluster ->
        ...
        val clusterText = clusterTp.joinToString("") { it.textPosition.unicode }
        val toAdd =
          TextWithCoordinates(
  ...
  private fun getAverageXAndY(textPositions: List<TextPosition>?): Pair<Double?, Double?> {
    val x = textPositions?.map { it.x }?.average()
    val y = textPositions?.map { it.y }?.average()
    return Pair(x, y)
  }
}
