...
class PdfPageProductsExtractor(
  ...
  fun getProductTextsWithSections(
    page: Int
    ...
    val initialCenters =
      pdfTextStripper.strippedTexts
        .filter { initialCenterPredicate.test(it) }
        .map { CentroidCluster<TextWithCoordinates>(it) }

    if (initialCenters.isEmpty()) {
      return emptyList()
    }

    val filteredStrippedTexts =
      pdfTextStripper.strippedTexts
        .filter { fontsToKeep.any { rgx -> it.font?.name?.contains(rgx) == true } }
        .filter { filterUnknownProductSection(it) }
        .toMutableList()
    ...
    val clusteredTextsWithSections =
      clusteredTexts.map { cluster ->
        val withSections = cluster.map { addProductSection(it) }
        val newPrices = withSections.filter { it.first == NEW_PRICE }.sortedBy { it.second.x }
        if (newPrices.size > 1) {
          ...
          val sectionsWithOneNewPrice =
            withSections
              .filter { twc -> twc.first != NEW_PRICE }
              .toMutableList()
              .plus(newPairNewPrice)
          ...
          if (maybeOldPriceToReplace == null) {
            ...
            sectionsWithOneNewPrice
              .filter { twc -> twc.first != OLD_PRICE }
    ...
    val validClusteredTexts =
      clusteredTextsWithSections.filter { cluster -> hasNameAndNewPrice(cluster) }

    // centers which were not matched
    val noMatchedCenters =
      initialCenters.filter {
        val centerText = it.center as TextWithCoordinates
        validClusteredTexts.flatten().none { pair -> pair.second == centerText }
      }

    // START -> logic for vertical products case
    val horizontalCentersEntry =
      noMatchedCenters.groupBy { it.center.point[1] }.entries.maxByOrNull { it.value.size }

    val horizontalCenters =
      horizontalCentersEntry
        ?.value
        ?.map { it.center as TextWithCoordinates }
        .orEmpty()
        .toMutableList()

    val additionalCenters =
      noMatchedCenters
        .map { it.center as TextWithCoordinates }
        .filter { it !in horizontalCenters }
        .filter {
          horizontalCentersEntry != null &&
            it.y != null &&
    ...
    if (horizontalCenters.size >= 3) {
      ...
      val stillNotMatchedCenters =
        noMatchedCenters.filter { it.center as TextWithCoordinates !in horizontalCenters }
      ...
      if (stillNotMatchedCenters.isNotEmpty()) {
        ...
        val allTextsSoFar = augmentedClusteredTexts.flatten().map { it.second }
        val leftTexts = filteredStrippedTexts.filter { it !in allTextsSoFar }.toMutableList()
        val kMeansPlus2 =
        ...
        val moreClusteredTexts =
          kMeansPlus2
            .cluster(leftTexts)
            .map { it.points }
            .map { cluster -> cluster.map { addProductSection(it) } }
            .filter { cluster -> hasNameAndNewPrice(cluster) }
  ...
  private fun hasNameAndNewPrice(
    cluster: List<Pair<ProductSection, TextWithCoordinates>>
  ): Boolean {
    val nameCount =
      cluster.count {
    ...
    val newPricesCount = cluster.count { it.first in newPricesProductSections }
    return nameCount >= 1 && newPricesCount == 1
  }

  private fun addProductSection(
    textWithCoordinates: TextWithCoordinates
  ): Pair<ProductSection, TextWithCoordinates> {
    val section =
      productSectionResolver.entries.first { (_, v) -> v.invoke(textWithCoordinates) }.key
  ...
  private fun getNameCoordinates(
    clusterWithSections: List<Pair<ProductSection, TextWithCoordinates>>
  ): TextWithCoordinates? {
    return clusterWithSections.firstOrNull { it.first == NAME }?.second
  }

  private fun combineTwoCloseToEachOtherPrices(
    prices: List<Pair<ProductSection, TextWithCoordinates>>
  ): String {
    return prices.sortedWith(compareBy({ it.second.x }, { it.second.y })).take(2).joinToString(
      separator = ""
    ) {
  ...
  private fun findClosestSections(
    ...
    return clusterWithSections
      .filter { it.first == sectionToFind }
      .sortedBy { abs(it.second.x.minus(coordinates?.x)) + abs(it.second.y.minus(coordinates?.y)) }
      .map { it.second }
      .take(toTake)
  }
