...
class TMarketProductsExtractor : UrlProductsExtractor {
    ...
    override fun extract(url: URL): List<Product> {
        ...
        return document.select("._products-list").select("div[data-box=product]")
            .mapNotNull {
                val productName = it.select("._product-name").text()

                val endDate = it.selectFirst("div[data-end-date]")?.attr("data-end-date")
                    ?.let { date ->
                        try {
                            LocalDateTime.parse(
                                date,
                ...
                val picUrl = it.selectFirst("._product-image img")?.attr("data-src")
                    ?.takeUnless { purl ->
