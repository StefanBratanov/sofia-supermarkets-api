...
class FantasticoProductsExtractor : PdfProductsExtractor {
      ...
      generateSequence(1) { it + 1 }
        ...
        .flatMap { pageNumber ->
          ...
          pageProductsExtractor.getProductTextsWithSections(pageNumber).mapNotNull {
            val name = getName(it)
            val oldPrice =
              it.firstOrNull { sectionAndText -> sectionAndText.first == OLD_PRICE }?.second?.text
            val newPrice =
              it
                .firstOrNull { sectionAndText ->
            ...
            val quantity =
              it
                .filter { sectionAndText -> sectionAndText.first == QUANTITY }
            ...
            Product(
              ...
              .takeIf { StringUtils.isNotBlank(name) }
          }
        }
        .toList()
  ...
  private fun getName(cluster: List<Pair<ProductSection, TextWithCoordinates>>): String {
    return cluster
      ...
      .filter { it.second.font?.name?.contains(myriadProRegex) == true }
      .joinToString(" ") { it.second.text.toString() }
      .let { regexesToRemove.fold(it) { name, toRemove -> name.replace(toRemove, "") } }
  }
}
