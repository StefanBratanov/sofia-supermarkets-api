...
internal class LidlFlowTest {

  @MockK lateinit var lidlSublinksScraper: LidlSublinksScraper
  ...
  @InjectMockKs lateinit var underTest: LidlFlow
  ...
  fun `runs flow for Lidl`() {
    ...
    val expectedToSave =
      ProductStore(supermarket = "Lidl", products = listOf(hello, world, foo, bar))
    verify {
  ...
  fun `gets correct supermarket name`() {
    assertThat(underTest.getSupermarket()).isEqualTo(Supermarket.LIDL)
  }
}
